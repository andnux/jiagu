...
class TinyPngTask extends DefaultTask {
    ...
    def run() {
        ...
        def compressedListFile = new File("${project.projectDir}/resource.json")
        if (!compressedListFile.exists()) {
            compressedListFile.createNewFile()
            compressedListFile.write('[]', StandardCharsets.UTF_8)
        } else {
            try {
                ...
                if (list instanceof ArrayList) {
                    ...
                    println("resource.json is invalid, ignore")
                }
            } catch (Exception ignored) {
                println("resource.json is invalid, ignore")
            }
